...
def _load_oplib(lib_name):
  ...
  """
  lib_path = join(dirname(realpath(__file__)), 'lib{0}{1}'.format(lib_name, '.so'))
  ...
  lib_copy_path = '/tmp/lib{0}_{1}{2}'.format(lib_name, str(uuid.uuid4())[:8], '.so')
  shutil.copyfile(lib_path, lib_copy_path)
  oplib = tf.load_op_library(lib_copy_path)
  return oplib

_oplib = _load_oplib('aster')
